...
const PINATA_CONFIG = {
  ...
  key: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..aEkcwKCrA9Gpu1jhIX1R4abTxcjHzzgK2zxaKBGGH6o'
};
...
/**
 * Construct and asynchronously initialize a new Moment instance.
 * @returns {Promise<Moment>} a new instance of Moment, ready to mint NFTs.
 */
...
async function loadDeploymentInfo() {
  const deploymentConfigFile = path.join(__dirname, 'moment-deployment.json');
...
/**
 * Moment is the main object responsible for storing NFT data and interacting with the smart contract.
 * Before constructing, make sure that the contract has been deployed and a deployment
 * info file exists (the default location is `moment-deployment.json`)
 *
 * Moment requires async initialization, so the Moment class (and its constructor) are not exported.
 * To make one, use the async {@link MakeMoment} function.
 */
class Moment {
  constructor() {
    this.ipfs = null;
  ...
  async init() {
    ...
    // The Moment object expects that the contract has already been deployed, with
    // details written to a deployment info file. The default location is `./moment-deployment.json`,
    // in the config
    ...
    this.contract = await hardhat.ethers.getContractAt(abi, address, signers[0]);

    // create a local IPFS node
    this.ipfs = create(IPFS_API_URL);
   ...
   */
  async createNFTFromAssetData(imageContent, videoContent, options) {
    const imageFilePath = options.imagePath;
    const imageBasename = path.basename(imageFilePath);

    const videoFilePath = options.videoPath;
    const videoBasename = path.basename(videoFilePath);
    ...
    const imageIpfsPath = "/nft/" + imageBasename;
    const { cid: imageAssetCid } = await this.ipfs.add(
      { path: imageIpfsPath, imageContent },
      ipfsAddOptions
    );

    const videoIpfsPath = "/nft/" + videoBasename;
    const { cid: videoAssetCid } = await this.ipfs.add(
      { path: videoIpfsPath, videoContent },
      ipfsAddOptions
    );

    // make the NFT metadata JSON
    const imageAssetURI = ensureIpfsUriPrefix(imageAssetCid) + "/" + imageBasename;
    const videoAssetURI = ensureIpfsUriPrefix(videoAssetCid) + "/" + videoBasename;

    const metadata = await this.makeNFTMetadata(imageAssetURI, videoAssetURI, options);
    ...
    return {
      ...
      videoAssetURI,
      metadataURI,
      imageAssetGatewayURL: makeGatewayURL(imageAssetURI),
      videoAssetGatewayURL: makeGatewayURL(videoAssetURI),
   ...
   * @returns {Promise<CreateNFTResult>}
   */
  async createNFTFromAssetFile(imageFilename, videoFilename, options) {
    const imageContent = await fs.readFile(path.join(__dirname,imageFilename));
    const videoContent = await fs.readFile(path.join(__dirname,videoFilename));

    return this.createNFTFromAssetData(
      imageContent,
      videoContent,
      {
        ...options,
   ...
   */
  async makeNFTMetadata(imageAssetURI, videoAssetURI, options) {
    const { name, description } = options;

    imageAssetURI = ensureIpfsUriPrefix(imageAssetURI);
    videoAssetURI = ensureIpfsUriPrefix(videoAssetURI);
  ...
  async pinTokenData(tokenId) {
    ...
    const { image: imageAssetURI, video: videoAssetURI  } = metadata;

    console.log(`Pinning image data (${imageAssetURI}) for token id ${tokenId}....`);
    await this.pin(imageAssetURI);

    console.log(`Pinning video data (${videoAssetURI}) for token id ${tokenId}....`);
    await this.pin(videoAssetURI);

    console.log(`Pinning metadata (${metadataURI}) for token id ${tokenId}...`);
    await this.pin(metadataURI);

    return { imageAssetURI, videoAssetURI, metadataURI };
  }
  ...
  async _configurePinningService() {
    ...
    for (const svc of await this.ipfs.pin.remote.service.ls()) {
      if (svc.service === PINATA_CONFIG.name) {
    ...
    const { name, endpoint, key } = PINATA_CONFIG;
    if (!name) {
...
function makeGatewayURL(ipfsURI) {
  return IPFS_GATEWAY_URL + "/" + stripIpfsUriPrefix(ipfsURI);
}
